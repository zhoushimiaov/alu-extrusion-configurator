...
const LS_KEY = { profile: 'alu.cfg.profile', rod: 'alu.cfg.rod', cart: 'alu.cfg.cart' };
...
const SCHEMA = {
  ...
  rod: ['schemaVersion', 'width', 'height', 'backType', 'shelf', 'wheels', 'color', 'bayCount'],
...
export function loadSaved(kind) {
  const activeKind = /^#rod/.test(location.hash) ? 'rod' : /^#cart/.test(location.hash) ? 'cart' : 'profile';
...
export function persist(kind, cfg) {
  ...
  const hashKind = /^#rod/.test(location.hash) ? 'rod' : /^#cart/.test(location.hash) ? 'cart' : 'profile';
  if (kind !== hashKind) return;
  const base = location.hash.replace(/[?&]c=[A-Za-z0-9_-]+/, '') || (hashKind === 'rod' ? '#rod' : hashKind === 'cart' ? '#cart' : '#');
