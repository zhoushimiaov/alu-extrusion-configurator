...
test('reveal animates position/scale only and never touches material opacity (materials stay shared)',()=>{const a=createAnims(new THREE.PerspectiveCamera(),controls());const cut=new THREE.MeshStandardMaterial();const brushed=new THREE.MeshStandardMaterial();const paneMat=new THREE.MeshStandardMaterial({transparent:true,opacity:.55});const p=new THREE.Mesh(new THREE.BoxGeometry(),[cut,brushed]);const pane=new THREE.Mesh(new THREE.BoxGeometry(),paneMat);a.reveal({p,pane});assert.equal(cut.opacity,1);assert.equal(brushed.opacity,1);assert.equal(paneMat.opacity,.55);assert.equal(p.scale.y<0.01,true);const saved=globalThis.performance;globalThis.performance={now:()=>saved.now()+10000};a.update();globalThis.performance=saved;assert.equal(p.scale.y,1);assert.equal(pane.scale.y,1);assert.equal(cut.transparent,false);assert.equal(paneMat.transparent,true);});
